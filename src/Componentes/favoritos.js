...
import EliminarComment from '../PopUps/EliminarComment_pop';

const FavoritosComp = () => {
  const [dropdownVisible, setDropdownVisible] = useState(false);
  const [dropdownDirection, setDropdownDirection] = useState('right'); // Default direction
  const [eliminarcomment, setEliminarComment] = useState(false); 
  ...
  return (
    <div className="max-w-sm rounded overflow-hidden shadow-lg bg-white m-4 relative">
      <div className="absolute top-0 right-0 mt-4 mr-4">
        ...
        {dropdownVisible && (
          <div className={`z-50 origin-top-${dropdownDirection} absolute mt-8 ${dropdownDirection === 'right' ? 'right-0' : 'left-0'} w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5`}>
      ...
      <div className="px-6 py-4 text-center">
        ...
        <div className="flex justify-between">
          ...
          <button className="bg-transparent hover:bg-rojo text-rojo font-semibold hover:text-white py-2 px-4 border border-rojo hover:border-transparent rounded w-5/12 mx-1" onClick={() => setEliminarComment(true)}>
            Share
          </button>
        </div>
      </div>
      {eliminarcomment && (
        <EliminarComment onClose={() => setEliminarComment(false)} />
      )}
    </div>
  );
};
